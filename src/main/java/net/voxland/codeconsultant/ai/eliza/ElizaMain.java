package net.voxland.codeconsultant.ai.eliza;
 ...
 *
 * Inspired From: http://chayden.net/eliza/Eliza.html
 */
 
 
 
 public class ElizaMain implements Serializable{

	private static final Log LOGGER = LogFactory.getLog(ElizaMain.class.getName());
	...
	// TODO: move it out
	public boolean fetchScript(boolean consultantMode){
		String scriptFilename="classpath:classic-script.txt";
		if(consultantMode) {
			scriptFilename="classpath:consultant-script.txt";
		}
		try {			
			File file = ResourceUtils.getFile(scriptFilename);
			InputStream inputScript = new FileInputStream(file);
			return readScript(inputScript)==0;
		} catch (IOException e) {
    ...
    public int readScript(InputStream inputScript) {
        DataInputStream in = new DataInputStream(inputScript);
        try {
            while (true) {
                String s;
                s = in.readLine();
			...
			LOGGER.error(e.getMessage());
			LOGGER.error("There was a problem reading the script file.");
            return 1;
        }
        return 0;
    }
